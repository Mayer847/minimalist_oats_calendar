...
class ScheduleEntry {
  ...
  const ScheduleEntry({required this.text, required this.colorValue, required this.status, required this.durationMinutes});
  ...
  factory ScheduleEntry.fromJson(Map<String, dynamic> json) => ScheduleEntry(
        ...
        status: EntryStatus.values.firstWhere((e) => e.name == json['status'], orElse: () => EntryStatus.none),
...
class UndoSnapshot {
  ...
  const UndoSnapshot(this.entries, this.extraLateRows);
}
...
class _WeeklySchedulePageState extends State<WeeklySchedulePage> {
  ...
  void initState() {
    ...
    _loadData();
    _startReviewTimer();
    WidgetsBinding.instance.addPostFrameCallback((_) => _initCloudSync());
  }
  ...
  DateTime _startOfWeek(DateTime d, int startWeekday) => _dateOnly(d).subtract(Duration(days: (_dateOnly(d).weekday - startWeekday) % 7));
  DateTime _weekEnd() => selectedWeekStart.add(const Duration(days: 6));
  List<DateTime> _visibleDates() => List.generate(7, (i) => selectedWeekStart.add(Duration(days: i)));
  ...
  List<int> _hourLabels() => List.generate((24 + extraLateRows) - startHour + 1, (i) => startHour + i);
  ...
  String _shortDate(DateTime date) => '${date.day.toString().padLeft(2, '0')}.${date.month.toString().padLeft(2, '0')}.${(date.year % 100).toString().padLeft(2, '0')}';
  String _dayLabel(DateTime date) => weekdayOptions.firstWhere((x) => x.weekday == date.weekday).label;
  ...
  void _rememberUndo() {
    undoStack.add(UndoSnapshot(Map<String, ScheduleEntry>.from(entries), extraLateRows));
  ...
  Future<void> _undo() async {
    ...
    setState(() {
      entries..clear()..addAll(snap.entries);
  ...
  void _ensureRowsFit(int maxEndMinutes) {
    ...
    extraLateRows = math.max(extraLateRows, math.max(3, requiredEndHour - 24));
  }

  void _resolveOverlapsForDate(String dateKey) {
    final day = entries.entries.where((e) => e.key.startsWith('$dateKey-')).toList()
    ...
    for (final item in day) {
      ...
      if (start < currentEnd) start = ((currentEnd + slotMinutes - 1) ~/ slotMinutes) * slotMinutes;
  ...
  Future<void> _loadData() async {
    ...
    if (savedEntries != null && savedEntries.trim().isNotEmpty) {
      ...
      entries..clear()..addAll(decoded.map((k, v) => MapEntry(k, ScheduleEntry.fromJson(Map<String, dynamic>.from(v as Map)))));
    }
    setState(() {
  ...
  Future<void> _saveData({bool scheduleCloud = true}) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('entries', jsonEncode(entries.map((k, v) => MapEntry(k, v.toJson()))));
  ...
  void _restoreFromBackupMap(Map<String, dynamic> backup) {
    ...
    final rawEntries = Map<String, dynamic>.from(backup['entries'] as Map? ?? {});
    setState(() {
      entries..clear()..addAll(rawEntries.map((k, v) => MapEntry(k, ScheduleEntry.fromJson(Map<String, dynamic>.from(v as Map)))));
      ...
      if (settings['selectedWeekStart'] != null) selectedWeekStart = _dateOnly(DateTime.parse(settings['selectedWeekStart'] as String));
  ...
  Future<void> _initCloudSync() async {
    ...
    try {
      ...
      if (mounted) setState(() => cloudSyncStatus = 'Connect Drive');
    } catch (e) {
  ...
  Future<void> _connectGoogleDrive() async {
    ...
    try {
      ...
      if (cloudBackup == null) {
        ...
        final choice = await showDialog<String>(
          ...
          builder: (_) => AlertDialog(
            ...
            actions: [
              TextButton(onPressed: () => Navigator.pop(context, 'local'), child: const Text('Keep local and overwrite cloud')),
              FilledButton(onPressed: () => Navigator.pop(context, 'cloud'), child: const Text('Restore cloud')),
            ],
          ),
        );
        if (choice == 'cloud') {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Google Drive sync error: $e')));
      }
    } finally {
  ...
  void _scheduleCloudSync() {
    ...
    cloudSyncDebouncer.run(() async {
      try {
        if (mounted) setState(() => cloudSyncStatus = 'Syncing...');
        await driveSync.saveBackup(_backupJson());
        if (mounted) setState(() => cloudSyncStatus = 'Synced');
      } catch (e) {
        debugPrint('Auto-sync failed: $e');
        if (mounted) setState(() => cloudSyncStatus = 'Sync failed');
      }
    });
  }

  Future<void> _exportJson() async {
    final bytes = Uint8List.fromList(utf8.encode(const JsonEncoder.withIndent('  ').convert(_backupJson())));
    _downloadBytes(bytes, 'oats_schedule_backup_${_shortDate(selectedWeekStart)}.json', 'application/json');
  }
  ...
  Future<void> _maybeAutoOpenReview() async {
    ...
    await _saveData();
    if (entries.keys.any((k) => k.startsWith('$todayKey-'))) await _reviewDate(_dateOnly(now));
  }
  ...
  Future<void> _editHeaderField(String title, String initialValue, ValueChanged<String> onSaved) async {
    final controller = TextEditingController(text: initialValue);
    final result = await showDialog<String>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text(title, style: _titleStyle),
        content: TextField(controller: controller, autofocus: true, style: GoogleFonts.caveat(fontSize: 28, shadows: const [])),
        actions: [
  ...
  Future<void> _editEntry(DateTime date, int startMinutes) async {
    ...
    final old = entries[key] ?? ScheduleEntry(text: '', colorValue: Colors.black.value, status: EntryStatus.none, durationMinutes: 60);
    ...
    final result = await showDialog<ScheduleEntry?>(
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              TextField(
                ...
                decoration: InputDecoration(hintText: 'Write the appointment / task...', hintStyle: GoogleFonts.caveat(fontSize: 27, color: Colors.black45, shadows: const [])),
              ),
              const SizedBox(height: 12),
              Text('Duration', style: _headerStyle),
              DropdownButton<int>(
                value: durationMinutes,
                items: const [30, 60, 90, 120, 150, 180, 240, 300, 360].map((m) => DropdownMenuItem(value: m, child: Text(_formatDuration(m)))).toList(),
                onChanged: (v) => v == null ? null : setDialogState(() => durationMinutes = v),
              ),
              ...
              Wrap(spacing: 10, children: [
                for (final color in const [Colors.black, Colors.red, Colors.blue, Colors.green, Colors.purple, Colors.orange, Colors.brown, Colors.teal])
                  GestureDetector(
                    ...
                    child: CircleAvatar(backgroundColor: color, child: selectedColor.value == color.value ? const Icon(Icons.check, color: Colors.white) : null),
                  ),
              ]),
            ]),
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
            TextButton(onPressed: () => Navigator.pop(context, ScheduleEntry(text: '', colorValue: selectedColor.value, status: EntryStatus.none, durationMinutes: durationMinutes)), child: const Text('Clear')),
            FilledButton(onPressed: () => Navigator.pop(context, old.copyWith(text: controller.text.trim(), colorValue: selectedColor.value, durationMinutes: durationMinutes)), child: const Text('Save')),
          ],
        ),
      ),
    );
  ...
  Future<void> _setEntryStatus(String key, ScheduleEntry entry, EntryStatus status, StateSetter setDialogState) async {
    _rememberUndo();
  ...
  Future<void> _reviewDate(DateTime date) async {
    ...
    final day = entries.entries.where((e) => e.key.startsWith('$dk-')).toList()..sort((a, b) => _minutesFromKey(a.key).compareTo(_minutesFromKey(b.key)));
    if (!mounted) return;
    if (day.isEmpty) {
      await showDialog(context: context, builder: (_) => AlertDialog(title: Text('Daily Review', style: _titleStyle), content: Text('No planned items for ${_dayLabel(date)} ${_shortDate(date)}.'), actions: [FilledButton(onPressed: () => Navigator.pop(context), child: const Text('OK'))]));
      return;
    }
    await showDialog(
      context: context,
                ...
                return Card(
                  ...
                  child: Padding(
                    ...
                    child: Wrap(spacing: 10, runSpacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [
                      SizedBox(width: 260, child: Text('${_formatSlotTime(start)} • ${_formatDuration(entry.durationMinutes)}\n${entry.text}', style: GoogleFonts.caveat(fontSize: 24, fontWeight: FontWeight.w700, color: Color(entry.colorValue), shadows: const []))),
                      FilledButton(onPressed: () => _setEntryStatus(key, entry, EntryStatus.done, setDialogState), child: const Text('Done')),
                      FilledButton.tonal(onPressed: () => _setEntryStatus(key, entry, EntryStatus.missed, setDialogState), child: const Text('Missed')),
                      OutlinedButton(onPressed: () => _setEntryStatus(key, entry, EntryStatus.unsure, setDialogState), child: const Text('Not sure')),
  ...
  int _slotFromLocalY(double y) => (y / slotHeight).floor().clamp(0, _totalSlots - 1).toInt();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(onPressed: cloudSyncBusy ? null : _connectGoogleDrive, icon: cloudSyncBusy ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.cloud_sync_outlined), label: Text(cloudSyncStatus)),
          TextButton.icon(onPressed: () async { setState(() => use24HourFormat = !use24HourFormat); await _saveData(); }, icon: const Icon(Icons.access_time), label: Text(use24HourFormat ? '24h' : '12h')),
          DropdownButton<int>(value: selectedStartWeekday, underline: const SizedBox.shrink(), items: weekdayOptions.map((x) => DropdownMenuItem(value: x.weekday, child: Text(x.label, style: GoogleFonts.caveat(fontSize: 22)))).toList(), onChanged: (v) async { if (v == null) return; setState(() { selectedStartWeekday = v; selectedWeekStart = _startOfWeek(selectedWeekStart, selectedStartWeekday); }); await _saveData(); }),
          IconButton(tooltip: 'Undo', onPressed: undoStack.isEmpty ? null : _undo, icon: const Icon(Icons.undo)),
          PopupMenuButton<String>(icon: const Icon(Icons.ios_share), onSelected: (v) { if (v == 'json_export') _exportJson(); if (v == 'json_import') _importJson(); if (v == 'png') _exportPng(); }, itemBuilder: (_) => const [PopupMenuItem(value: 'json_export', child: Text('Export backup JSON')), PopupMenuItem(value: 'json_import', child: Text('Import backup JSON')), PopupMenuItem(value: 'png', child: Text('Download schedule PNG'))]),
      ...
      body: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        _buildHeader(),
        Expanded(child: Scrollbar(thumbVisibility: true, child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: SingleChildScrollView(child: _buildScheduleTable())))),
      ]),
    );
  }
  ...
  Widget _buildHeader() => Padding(
        ...
        child: Wrap(spacing: 16, runSpacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [
          Row(mainAxisSize: MainAxisSize.min, children: [
            IconButton(onPressed: () async { setState(() => selectedWeekStart = selectedWeekStart.subtract(const Duration(days: 7))); await _saveData(); }, icon: const Icon(Icons.chevron_left)),
            InkWell(onTap: () async { setState(() => selectedWeekStart = _startOfWeek(DateTime.now(), selectedStartWeekday)); await _saveData(); }, child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 5), decoration: BoxDecoration(color: Colors.white.withOpacity(0.45), border: Border.all(color: Colors.black, width: 1.2)), child: Text('${_shortDate(selectedWeekStart)} ~ ${_shortDate(_weekEnd())}', style: GoogleFonts.caveat(fontSize: 31, fontWeight: FontWeight.w700, shadows: const [])))),
            IconButton(onPressed: () async { setState(() => selectedWeekStart = selectedWeekStart.add(const Duration(days: 7))); await _saveData(); }, icon: const Icon(Icons.chevron_right)),
          ]),
          InkWell(onTap: () => _editHeaderField('TEAM', team, (v) => team = v), child: Text('TEAM : ${team.isEmpty ? '__________' : team}', style: _headerStyle)),
          InkWell(onTap: () => _editHeaderField('NAME', name, (v) => name = v), child: Text('NAME : ${name.isEmpty ? '__________' : name}', style: _headerStyle)),
        ]),
      );
    ...
    return RepaintBoundary(
      ...
      child: ColoredBox(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(margin: const EdgeInsets.all(12), decoration: BoxDecoration(border: Border.all(color: Colors.black, width: 1.5)), child: Column(children: [
            Row(children: [_headerCell('', timeColumnWidth), for (final d in dates) _dayHeaderCell(d)]),
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [_timeColumn(), for (final d in dates) _dayColumn(d)]),
          ])),
          Padding(padding: const EdgeInsets.fromLTRB(12, 0, 12, 24), child: Wrap(spacing: 10, children: [
            OutlinedButton.icon(onPressed: () async { _rememberUndo(); setState(() => extraLateRows++); await _saveData(); }, icon: const Icon(Icons.add), label: Text('Add late row', style: GoogleFonts.caveat(fontSize: 24))),
            if (extraLateRows > 0) OutlinedButton.icon(onPressed: () async { _rememberUndo(); setState(() => extraLateRows--); await _saveData(); }, icon: const Icon(Icons.remove), label: Text('Remove late row', style: GoogleFonts.caveat(fontSize: 24))),
          ])),
        ]),
      ),
    );
  ...
  Widget _headerCell(String text, double width) => Container(width: width, height: headerHeight, alignment: Alignment.center, decoration: BoxDecoration(border: Border.all(color: Colors.black, width: 0.8)), child: Text(text, textAlign: TextAlign.center, style: _headerStyle.copyWith(height: 0.9)));

  Widget _dayHeaderCell(DateTime date) => GestureDetector(onTap: () => _reviewDate(date), child: Container(width: dayColumnWidth, height: headerHeight, alignment: Alignment.center, decoration: BoxDecoration(border: Border.all(color: Colors.black, width: 0.8)), child: Text('${_dayLabel(date)}\n${date.day.toString().padLeft(2, '0')}', textAlign: TextAlign.center, style: _headerStyle.copyWith(height: 0.9))));
  ...
  Widget _timeColumn() => Container(
        ...
        child: CustomPaint(painter: RowDashPainter(totalSlots: _totalSlots, slotHeight: slotHeight, color: Colors.black54, showHourOnly: true), child: Stack(children: [for (final h in _hourLabels()) Positioned(top: (h - startHour) * 2 * slotHeight, left: 0, right: 0, height: slotHeight * 2, child: Center(child: Text(_formatHourLabel(h), style: _headerStyle)))])),
      );

  Widget _dayColumn(DateTime date) {
    final dk = _dateKey(date);
    final day = entries.entries.where((e) => e.key.startsWith('$dk-')).toList()..sort((a, b) => _minutesFromKey(a.key).compareTo(_minutesFromKey(b.key)));
    return Builder(builder: (targetContext) => DragTarget<EntryDragData>(
          onAcceptWithDetails: (details) {
            final box = targetContext.findRenderObject() as RenderBox;
            final local = box.globalToLocal(details.offset);
            _handleDrop(details.data, date, startHour * 60 + _slotFromLocalY(local.dy) * slotMinutes);
          },
          builder: (context, candidates, rejected) => GestureDetector(
            behavior: HitTestBehavior.opaque,
            onTapUp: (details) => _editEntry(date, startHour * 60 + _slotFromLocalY(details.localPosition.dy) * slotMinutes),
            child: Container(width: dayColumnWidth, height: _bodyHeight, decoration: const BoxDecoration(border: Border(left: BorderSide(color: Colors.black, width: 0.8), right: BorderSide(color: Colors.black, width: 0.8))), child: CustomPaint(painter: RowDashPainter(totalSlots: _totalSlots, slotHeight: slotHeight, color: Colors.black38), child: Stack(clipBehavior: Clip.none, children: [if (candidates.isNotEmpty) Positioned.fill(child: ColoredBox(color: Colors.greenAccent.withOpacity(0.08))), for (final e in day) _entryBlock(date, e.key, e.value)]))),
          ),
        ));
  }
  ...
  Widget _entryBlock(DateTime date, String key, ScheduleEntry entry) {
    ...
    Widget block([double opacity = 1]) => Opacity(opacity: opacity, child: GestureDetector(onTap: () => _editEntry(date, start), child: Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 3), decoration: BoxDecoration(color: _statusBackground(entry.status), border: Border.all(color: Color(entry.colorValue), width: 1.3), borderRadius: BorderRadius.circular(6)), child: Align(alignment: Alignment.topCenter, child: Text(entry.text, textAlign: TextAlign.center, maxLines: math.max(1, durationSlots), overflow: TextOverflow.ellipsis, style: _cellStyle.copyWith(color: Color(entry.colorValue)))))));
    return Positioned(top: top, left: 4, right: 4, height: height, child: LongPressDraggable<EntryDragData>(data: EntryDragData(date, start, entry), feedback: Material(color: Colors.transparent, child: SizedBox(width: dayColumnWidth - 8, height: math.min(height, 90), child: block(0.88))), childWhenDragging: block(0.35), child: block()));
  }
}
...
class RowDashPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = color..strokeWidth = 0.7;
    for (var slot = 1; slot < totalSlots; slot++) {
      ...
      for (double x = 5; x < size.width - 5; x += dash + gap) {
        canvas.drawLine(Offset(x, y), Offset(math.min(x + dash, size.width - 5), y), paint);
      }
    }
  }

  @override
  bool shouldRepaint(covariant RowDashPainter old) => old.totalSlots != totalSlots || old.slotHeight != slotHeight || old.color != color || old.showHourOnly != showHourOnly;
}
