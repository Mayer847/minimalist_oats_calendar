// Google Drive appDataFolder sync helper for Minimalist OATS Calendar.
// This file is intentionally plain browser JS so Flutter Web can call it via dart:js_util.
...
window.oatsGoogleDriveInit = async function(clientId) {
  ...
  window.oatsGoogleDrive.tokenClient = google.accounts.oauth2.initTokenClient({
    client_id: clientId,
    scope: OATS_DRIVE_SCOPE,
    callback: () => {},
  });
...
window.oatsGoogleDriveIsSignedIn = async function() {
  return !!window.oatsGoogleDrive.accessToken;
};
...
async function findBackupFile() {
  ...
  const response = await gapi.client.drive.files.list({
    ...
    q: `name='${state.backupFileName}' and trashed=false`,
  ...
  if (files.length > 0) {
    state.backupFileId = files[0].id;
    return state.backupFileId;
  }
  return null;
}
...
window.oatsGoogleDriveLoadBackup = async function() {
  ...
  if (!fileId) {
    state.status = 'Connected - no cloud backup yet';
    return null;
  }
...
window.oatsGoogleDriveGetStatus = async function() {
  return window.oatsGoogleDrive.status;
};
