...
class GoogleDriveSyncService {
  ...
  bool get isConfigured => clientId.isNotEmpty && !clientId.contains('PASTE_YOUR_GOOGLE_CLIENT_ID_HERE');

  Future<void> initialize() async {
    if (!isConfigured) {
      throw StateError('GOOGLE_CLIENT_ID is missing. Run/build with --dart-define=GOOGLE_CLIENT_ID=xxxxx.apps.googleusercontent.com');
    }
    await _callPromise('oatsGoogleDriveInit', [clientId]);
  }

  Future<String?> connect() async => (await _callPromise('oatsGoogleDriveConnect', []))?.toString();

  Future<void> signOut() async => _callPromise('oatsGoogleDriveSignOut', []);
  ...
  Future<String> getStatus() async => (await _callPromise('oatsGoogleDriveGetStatus', []))?.toString() ?? 'Disconnected';

  Future<dynamic> _callPromise(String method, List<dynamic> args) async {
    final fn = js_util.getProperty(js_util.globalThis, method);
    if (fn == null) throw StateError('$method is not loaded. Check web/google_drive_sync.js and web/index.html.');
    return js_util.promiseToFuture(js_util.callMethod(js_util.globalThis, method, args));
  }
}
